...
export function UltimateSalaryCalculator({ onBack }: UltimateSalaryCalculatorProps) {
  // State für Berechnungsparameter
  ...
  const calculateNettoBrutto = useCallback(() => {
    ...
    const result = calculateNetToGross({
      targetNetMonthly: targetNet,
      ...baseParams,
    });
    setNettoGrossResult(result);
  }, [targetNetSalary, baseParams]);

  // Slider-Berechnung in Echtzeit
  ...
  // Gehaltskurve für Visualisierung
  ...
  // Gehaltserhöhungsanalyse
  const raiseAnalysis = useMemo(() => {
    const currentGross = mode === 'slider' ? sliderValue[0] : (parseFloat(grossSalary) || 3500);
    return analyzeRaise(currentGross, 500, baseParams);
  }, [baseParams, grossSalary, sliderValue, mode]);

  // Auto-Berechnung bei Änderungen
  useEffect(() => {
    if (mode === 'brutto-netto') {
      calculateBruttoNetto();
    } else if (mode === 'netto-brutto') {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Präzise Berechnung • Netto-Brutto Umkehr • Optimierungstipps
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        <Button
          variant={mode === 'brutto-netto' ? 'default' : 'outline'}
          onClick={() => setMode('brutto-netto')}
        ...
        <Button
          variant={mode === 'netto-brutto' ? 'default' : 'outline'}
          onClick={() => setMode('netto-brutto')}
          className="flex items-center gap-2"
        >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-1 space-y-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              {mode === 'slider' ? (
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Wie viel möchtest du auf dem Konto haben?
                  </p>
                </div>
              ) : (
                <div className="space-y-2">
              ...
              {mode !== 'slider' && (
                <Button 
                  onClick={mode === 'netto-brutto' ? calculateNettoBrutto : calculateBruttoNetto} 
                  className="w-full"
                >
            ...
            <Card className="border-primary/20 bg-primary/5">
              ...
              <CardContent className="text-sm space-y-1">
                <div className="flex justify-between">
                  <span>Netto-Zuwachs:</span>
                  <span className="font-medium text-primary">
                    +{formatCurrency(raiseAnalysis.netIncrease)}
                  </span>
                </div>
                <div className="flex justify-between text-muted-foreground">
                  <span>Davon gehen weg:</span>
                  <span>{formatCurrency(raiseAnalysis.taxOnRaise)}</span>
                </div>
                <div className="flex justify-between text-muted-foreground">
                  <span>Du behältst:</span>
                  <span>{raiseAnalysis.percentageKept.toFixed(1)}%</span>
                </div>
              </CardContent>
            </Card>
          )}
        ...
        {/* Mittlere + Rechte Spalte: Ergebnisse */}
        <div className="xl:col-span-2 space-y-4">
              ...
              <CardHeader className="bg-gradient-to-r from-primary/10 to-primary/5 border-b">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      {mode === 'netto-brutto' 
                        ? `Für ${formatCurrency(parseFloat(targetNetSalary) || 0)} netto brauchst du:`
                        : 'Nach Abzug aller Steuern und Sozialabgaben'}
                    </CardDescription>
                  </div>
                  {mode === 'netto-brutto' && nettoGrossResult && (
                    <Badge variant="outline" className="text-xs">
                      {nettoGrossResult.iterations} Iterationen
                    </Badge>
                  )}
                </div>
              </CardHeader>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  <div className="text-center p-4 rounded-lg bg-muted/50">
                    ...
                    <p className="text-2xl font-bold">
                      {mode === 'netto-brutto' && nettoGrossResult 
                        ? formatCurrency(nettoGrossResult.requiredGross)
                        : formatCurrency(currentResult.grossMonthly)}
                    </p>
                  </div>
                  ...
                  <div className="text-center p-4 rounded-lg bg-muted/50">
                    ...
                    <p className="text-2xl font-bold">
                      {((currentResult.netMonthly / currentResult.grossMonthly) * 100).toFixed(1)}%
                    </p>
                  </div>
                  <div className="text-center p-4 rounded-lg bg-muted/50">
                    <p className="text-sm text-muted-foreground">AG-Kosten</p>
                    <p className="text-2xl font-bold text-muted-foreground">
                      {formatCurrency(currentResult.employerCosts / 12)}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="text-destructive">-{formatCurrency(currentResult.healthInsurance / 12)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Rentenversicherung</span>
                        <span className="text-destructive">-{formatCurrency(currentResult.pensionInsurance / 12)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Arbeitslosenversicherung</span>
                        <span className="text-destructive">-{formatCurrency(currentResult.unemploymentInsurance / 12)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Pflegeversicherung</span>
                        <span className="text-destructive">-{formatCurrency(currentResult.careInsurance / 12)}</span>
                      </div>
                      <div className="flex justify-between font-medium pt-1 border-t">
                        <span>Summe SV</span>
                        <span className="text-destructive">-{formatCurrency(currentResult.totalSocialContributions / 12)}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="text-destructive">-{formatCurrency(currentResult.incomeTax / 12)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Solidaritätszuschlag</span>
                        <span className="text-destructive">-{formatCurrency(currentResult.solidarityTax / 12)}</span>
                      </div>
                      {churchTax && currentResult.churchTax > 0 && (
                        <div className="flex justify-between">
                          <span>Kirchensteuer</span>
                          <span className="text-destructive">-{formatCurrency(currentResult.churchTax / 12)}</span>
                        </div>
                      )}
                      <div className="flex justify-between font-medium pt-1 border-t">
                        <span>Summe Steuern</span>
                        <span className="text-destructive">-{formatCurrency(currentResult.totalTaxes / 12)}</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {currentResult && (
            <Tabs defaultValue="curve" className="w-full">
              <TabsList className="grid grid-cols-2 w-full max-w-md">
                ...
                <TabsTrigger value="marginal" className="flex items-center gap-2">
              ...
              <TabsContent value="curve" className="mt-4">
                <SalaryCurveChart 
                  data={salaryCurve} 
                  currentGross={mode === 'slider' ? sliderValue[0] : (parseFloat(grossSalary) || undefined)}
                  height={280}
                />
              </TabsContent>
          ...
          {/* Optimierungstipps */}
          {currentResult && showOptimization && (
